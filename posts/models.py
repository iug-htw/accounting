...
class Aufgabe_neu(models.Model):
    ...
    unternehmen_kategorie = models.ForeignKey(Unternehmen, on_delete=models.CASCADE,help_text="Das Unternehmen oder die Fallstudie, zu welcher die Aufgabe zugeordnet wird.")
    fragentyp = models.ForeignKey(Aufgabenkategorie,null=True, on_delete=models.CASCADE, help_text="Der Aufgabentyp; Was für eine Art von Aufgabe liegt vor?")
    ersteller = models.IntegerField(null=True, blank=True)
    unterkategorie = models.IntegerField(null = True, blank=True, default = 1,help_text="Feld für Unterteilung von Fragen mit dem selben Fragentyp")
    fragentyp_text = models.CharField(null = True,max_length=255) #kann weg
    mailtext = models.TextField(help_text="Der angezeigte Text in der Mail, welchen die Nutzer für jede Aufgabe erhalten.")
    nutzungsdauer = models.IntegerField(null=True, blank=True)
    feedback_konto_falsch = models.TextField(null=True, blank=True, help_text="Ein allgemeines Feedback für eine falsche Lösung hinsichtlich der ausgewählten Konten.")
    feedback_betrag_falsch = models.TextField(null=True, blank=True, help_text="Ein allgemeines Feedback für eine falsche Lösung hinsichtlich des Betrages.")
    ...
    eigene_ansicht = models.TextField(blank=True, null=True, default='Secure Net\nTreskowallee 8\n10318 Berlin',help_text="Angezeigter Text in der Mail, welche für die Nutzer verschickt wird. Zeigt die Addresse des eigenen Unternehmens")
    ...
    zahlweise = models.CharField(max_length=255, blank=True, null=True,help_text="Auf welche Weise wird gezahlt? (Bar, Überweisung...)")
    verabschiedung = models.TextField(blank=True, null=True, default='Mit freundlichen Grüßen\nIhr Unternehmen',help_text="Angezeigter Text in der Mail. Zeigt die Verabschiedung. ")
    kontakt = models.TextField(blank=True, null=True, default='Tel: 01234 567890\nE-Mail: info@unternehmen.de',help_text="Angezeigter Text in der Mail. Zeigt die Kontaktdaten des eigenen Unternehmens")
