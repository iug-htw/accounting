...
from posts.models import Buchung, Konto
...
for buchung in Buchung.objects.all():
    soll_raw = json.loads(buchung.antwort_konten_soll or "[]")
    haben_raw = json.loads(buchung.antwort_konten_haben or "[]")
    ...
    if neue_soll != soll_raw or neue_haben != haben_raw:
        buchung.antwort_konten_soll = json.dumps(neue_soll)
        buchung.antwort_konten_haben = json.dumps(neue_haben)
        buchung.save()
        anzahl += 1
